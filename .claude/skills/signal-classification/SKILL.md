...
# n > ~1000 calibration samples: isotonic
calibrated = CalibratedClassifierCV(base_model, cv=5, method="isotonic")
# n < ~1000 calibration samples: sigmoid (Platt) — see below
...
**Pick the method by sample size, not by model family.** Isotonic
calibration beats Platt scaling for tree models *only when there are enough
calibration samples* — roughly **n > 1000**. Isotonic fits a
piecewise-constant, non-parametric step function with no shape assumption,
so below that it overfits the calibration set badly and produces worse
probabilities than the two-parameter sigmoid it replaced. Platt/sigmoid
scaling assumes a shape and pays for that assumption with far lower
variance, which is the right trade at small n.

Trading datasets routinely land on the small side of that line: a strategy
with a few hundred labeled firings has nowhere near enough data for
isotonic, regardless of the fact that the base model is a tree ensemble.

**Reference implementation in this repo:** `engine/te/ml/calibrate.py` uses
`method="sigmoid"` exclusively and is deliberate, not an oversight — its
training sample is a few hundred firings. A test in
`engine/tests/ml/test_calibrate.py` greps every module under `te/ml/` to
ensure isotonic is never reintroduced. Do not "upgrade" it to isotonic on
the strength of the tree-model rule of thumb alone; check n first.
