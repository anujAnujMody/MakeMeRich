...
> **Exception — `engine/te/ml/` (trading ML).** Model choice there is
> governed by the more specific **`signal-classification`** skill ("use
> XGBoost for small trading datasets <10k samples"), not this stack-wide
> default. `engine/te/ml/train.py` uses XGBoost deliberately and documents
> why in its module docstring. Leave it alone.
