...
- [`scikit-learn`](references/scikit-learn.md) — tabular ML
  ...
  > **Carve-out — `engine/te/ml/` (trading ML) is out of scope for this
  > default.** Model choice under `engine/te/ml/` is governed by the more
  > specific **`signal-classification`** skill, which calls for XGBoost on
  > small (<10k sample) trading datasets. That code uses XGBoost
  > deliberately, with the rationale written out in
  > `engine/te/ml/train.py`'s module docstring. Do not "helpfully" migrate
  > it to `HistGradientBoostingClassifier` on the strength of this
  > stack-wide default — the specific skill wins.
