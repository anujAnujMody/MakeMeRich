"""`add_subscriptions` must reach a LIVE connection, not wait for a drop.
...
Recording a strike band fixes the coverage, but a band costs one
rate-limited broker call per strike to resolve. That cannot sit in front of
the connection carrying the index bars, so the strikes have to join a feed
that is already up — which `subscribe()` alone cannot do, since it only
...
async def _run_client_until(*, initial: list[Instrument], after_connect: list[Instrument]) -> list[dict[str, object]]:
    ...
    # Only instruments NOT already registered produce a frame — the dedup is
    # itself under test, so this count must exclude repeats rather than
    # hanging when one is correctly suppressed.
    fresh = [inst for inst in after_connect if inst not in initial]
    expected = len(initial) + 1 + len(fresh)
    ...
    try:
        ...
        await client.add_subscriptions(after_connect)
...
async def test_a_late_instrument_is_replayed_on_reconnect_too() -> None:
    ...
    await client.add_subscriptions([option])
